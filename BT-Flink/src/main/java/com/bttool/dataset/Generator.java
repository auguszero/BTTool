...
public class Generator implements InputFormat<Tuple2<String, Integer>, GenericInputSplit> {
	...
	public static Generator generate(long numKeys, int recordsPerKey,String sourcename) {
		return new Generator(numKeys, recordsPerKey, false,sourcename);
	}

	public static Generator generateInfinitely(long numKeys,String sourcename) {
		return new Generator(numKeys, 0, true,sourcename);
	}

	private Generator(long numKeys, int recordsPerKey, boolean infinite,String sourcename) {
	...
	public Tuple2<String, Integer> nextRecord(Tuple2<String, Integer> reuse) throws IOException {
		...
		reuse.f0 = key;
		reuse.f1 = filterVal;
		System.out.println("sourcename -> "+sourcename+" tuple ->"+key + " <->" + filterVal);
		return reuse;
	}
